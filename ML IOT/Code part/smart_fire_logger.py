...
def ensure_prerequisites():
    ...
    if not file_has_expected_header(LOG_CSV):
...
def ldr_value(flame):
    if flame:
        return round(random.uniform(50, 350), 0)
    return round(random.uniform(900, 3600), 0)


def flicker_value(flame):
    if flame:
        return round(random.uniform(250, 850), 1)
...
def rule_override(temp, hum, smoke_adc, co_adc, flame, ldr_flicker):
    """
    Return (override_label, reason) or (None, None) to defer to ML.
    ADC scale is 0-4095.
    """
...
def main():
    ...
    try:
        while True:
            ...
            simulator = random.choices(SIMULATORS, weights=WEIGHTS, k=1)[0]
            scenario, temp, hum, smoke_adc, co_adc, flame, ldr_adc, ldr_flicker, motion = (
                simulator()
            )

            features = [[temp, hum, smoke_adc, co_adc, flame, ldr_adc, ldr_flicker, motion]]
            ml_pred = int(model.predict(features)[0])

            override, reason = rule_override(
                temp, hum, smoke_adc, co_adc, flame, ldr_flicker
            )

            if override is not None:
                final_label = override
                final_reason = reason
            elif ml_pred == 2 and hum > 65 and flame == 0 and co_adc < 600:
                final_label = 1
