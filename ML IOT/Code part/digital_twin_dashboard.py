...
app.layout = html.Div(
    style={"background": BG, "minHeight": "100vh", "padding": "32px 40px"},
    children=[
        html.Div(
            [
                html.Div(
        ...
        html.Div(
            [
                ...
                html.Div(
                    dcc.Graph(id="gauge-smoke", config={"displayModeBar": False}),
                    style={"flex": "1"},
                ),
                html.Div(
                    dcc.Graph(id="gauge-co", config={"displayModeBar": False}),
                    style={"flex": "1"},
                ),
                html.Div(
                    dcc.Graph(id="gauge-flicker", config={"displayModeBar": False}),
                    style={"flex": "1"},
                ),
            ],
        ...
        html.Div(
            [
                html.Div(
                    card(
                        dcc.Graph(id="chart-temp-smoke", config={"displayModeBar": False})
                    ),
                    style={"flex": "1"},
                ),
                html.Div(
        ...
        html.Div(
            [
                ...
                html.Div(
                    card(
                        dcc.Graph(
                            id="chart-label-history", config={"displayModeBar": False}
                        )
                    ),
                    style={"flex": "2"},
                ),
...
PLOT_LAYOUT = dict(
    ...
    xaxis=dict(gridcolor=BORDER, showgrid=True, zeroline=False, tickfont=dict(size=10)),
    yaxis=dict(gridcolor=BORDER, showgrid=True, zeroline=False, tickfont=dict(size=10)),
    height=220,
)
...
def make_gauge(value, title, max_val, color):
    fig = go.Figure(
        go.Indicator(
            ...
            title={
                "text": title,
                "font": {"family": FONT, "color": TEXT_DIM, "size": 11},
            },
            gauge={
                "axis": {
                    "range": [0, max_val],
...
def refresh(_):
    ...
    if "Final_Label" in df.columns:
        label_values = [label_map.get(str(value).upper(), 0) for value in df["Final_Label"]]
        label_colors = [
            SAFE if value == 0 else WARN if value == 1 else FIRE_CLR
    ...
    try:
        gauge_temp = make_gauge(latest["Temperature"], "TEMPERATURE C", 200, badge_color)
        gauge_smoke = make_gauge(latest["Smoke_ADC"], "SMOKE ADC", 4095, badge_color)
        gauge_co = make_gauge(latest["CO_ADC"], "CO ADC", 4095, badge_color)
        gauge_flicker = make_gauge(
            latest["LDR_Flicker"], "LDR FLICKER", 1000, badge_color
        )
    except Exception:
        gauge_temp = gauge_smoke = gauge_co = gauge_flicker = empty_fig()

    ml_raw = latest.get("ML_Pred", PLACEHOLDER)
    if str(ml_raw).isdigit():
        ml_display = {0: "NORMAL", 1: "WARNING", 2: "FIRE"}.get(int(ml_raw), str(ml_raw))
    else:
        ml_display = str(ml_raw)
